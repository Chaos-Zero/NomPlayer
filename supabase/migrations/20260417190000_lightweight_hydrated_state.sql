...
--
-- This version joins track_sources directly (no view overhead) and returns
-- only the fields the client cannot derive locally: videoId, trackId, and
-- supportLevel (for support entries). The client resolves the rest from cache.
...
BEGIN
  ...
  noms AS (
    SELECT
      ts.source_external_id AS "videoId",
