-- Previously, buildReconcileEntries (src/lib/vgmcIngest.js) excluded a
-- dropped-and-zeroed-out nomination entirely, and this function's trailing
-- DELETE then removed its user_playlist_tracks row outright the next time
-- its thread synced - game/song/support history gone, not just hidden.
--
-- buildReconcileEntries now emits every ever-validly-nominated record,
-- active or not, tagged with an `is_active` flag (see its docstring). This
-- function uses that flag to mark a row dropped (is_dropped = true) instead
-- of never inserting/updating it, so its history survives to be shown again
-- behind the frontend's "show dropped nominations" toggle - it just stops
-- counting toward the live playlist/standings by default (see
-- fetchVgmcPlaylistTracks/toPlaylistVideos in src/lib/vgmcStandings.js and
-- the sidebarTracks/playingTracks filtering in App.jsx).
--
-- Because dropped rows now always reappear in entries_input (and get
-- UPDATEd, not omitted), the trailing DELETE below effectively only fires
-- for a source_key that fell out of the batch entirely - a retired-track
-- conflict or a video-id collision skip - genuinely-gone cases, not dropped
-- ones. No change needed there beyond this comment.
