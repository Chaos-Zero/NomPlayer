-- Fix public.import_vgmc_catalog_row to correctly update existing primary sources
-- This version explicitly updates the primary source for a given track_id if provided
-- avoiding duplicate primary sources or unique constraint violations.
...
BEGIN
  ...
  -- 3. Update Primary Source explicitly if track_id_input is provided
  IF track_id_input IS NOT NULL THEN
    -- Check if ANY source for this track matches the video_id already (even if not primary)
    -- to avoid violating track_sources_external_source_unique index
    IF EXISTS (SELECT 1 FROM public.track_sources WHERE provider = 'youtube' AND external_id = youtube_video_id_input AND track_id != resolved_track_id) THEN
        RAISE EXCEPTION 'A different track already uses the video id %', youtube_video_id_input;
    END IF;

    -- Update the primary source for this track to the new ID/URL
    UPDATE public.track_sources
    SET
      external_id = youtube_video_id_input,
      source_url = canonical_source_url,
      submitted_url = normalized_submitted_url,
      updated_at = timezone('utc', now())
    WHERE track_id = resolved_track_id AND is_primary = true;

    -- If no primary source was found (weird, but possible), insert one
    IF NOT FOUND THEN
