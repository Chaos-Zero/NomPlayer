...
BEGIN
  ...
  IF track_id_input IS NOT NULL THEN
    ...
    ON CONFLICT (track_id, provider) WHERE is_primary = true -- Hypothetical index, using fallback below if not exists
    DO UPDATE SET
  ...
  -- 4. Upsert Appearances (omitted for brevity in this fix or re-included as needed)
  -- The rest of the function remains same...
  -- (Actually let's keep it complete to avoid breaking things)
