-- Create a new migration to update the track_catalog view to include all sources.
-- This ensures that any known URL (videoId) for a track correctly displays its community statistics.
...
-- Recreate track_catalog view to include ALL sources per track
...
SELECT
  ...
  track_sources.id AS source_id,
  track_sources.id AS primary_source_id, -- Alias for backward compatibility
  ...
  track_sources.is_primary, -- Include the flag just in case
  COALESCE(
    (
      SELECT jsonb_agg(
        jsonb_build_object(
  ...
  EXISTS (
    ...
    FROM public.track_tournament_appearances appearances
    WHERE appearances.track_id = tracks.id
      AND appearances.placement IS NOT NULL
  ) AS has_result
FROM public.tracks
LEFT JOIN public.tournaments retired_tournament
  ON retired_tournament.id = tracks.retired_by_tournament_id
INNER JOIN public.track_sources -- Changed to INNER JOIN since we need at least one source for search/mapping
  ON track_sources.track_id = tracks.id;

-- Recreate search_track_catalog function to include new columns
...
AS $$
  ...
  SELECT
    ...
    source_id,
    primary_source_id, -- matching new column name
    provider,
    source_external_id,
    source_url,
    submitted_url,
    ...
    last_fetched_at,
    is_primary, -- matching new column name
    tournaments,
    tournament_count,
    support_count_1,
    support_count_2,
    ...
    has_result
  FROM ranked_catalog
  -- Use DISTINCT ON track_id to only show one source per track in search results (preferring primary)
  -- Actually, search results often need to show specific sources if that's what was matched.
  -- But for simplicity, we favor is_primary.
  ORDER BY track_id, is_primary DESC, search_rank DESC, display_title ASC
