-- When updating a track's URL to a new video ID, the old primary source was not
-- demoted before inserting the new one, violating track_sources_primary_per_track_idx.
-- Fix: demote existing primary sources for the track before upserting the new one.
...
BEGIN
  ...
  -- Without this, inserting a new primary source violates track_sources_primary_per_track_idx.
  UPDATE public.track_sources
